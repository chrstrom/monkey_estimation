...
if task == 'a':
    ...
    plt.plot([-10, 60], [cfg.f0, cfg.f0], 'k')
    for i in range(M):
        n = range(N*i, N*(i+1))
        #plt.semilogy(SNRs[1:], np.abs(np.diff(mean_w[n])), '.--')
        plt.plot(SNRs, mean_f[n] / (2*np.pi), '.--')
        plt.ylim([99925, 100100])
...
if task == 'b':
    # format: SNR_dB, K, crlb_w, var_w, w_estimate_valid, crlb_phi, var_phi, phi_estimate_valid, mean_w, mean_phi
    with open('./data/part_b_run_6_N_1000.csv') as csvfile:
    ...
    plt.semilogy(SNRs, crlb_w[0:N], 'k.:')
    plt.semilogy(SNRs, var_w[0:N], 'r.-')
    ...
    plt.semilogy(SNRs, crlb_phi[0:N], 'k.:')
    plt.semilogy(SNRs, var_phi[0:N], 'r.-')
    plt.legend(['CRLB', 'Estimator'])
    plt.xlabel("SNR")
    plt.ylabel("Variance")
